...
const Navigation = forwardRef(({ sectionsOnPage }, ref) => {
  const { domain: { site: { siteMetadata: { menuLinks } } } } = useContext(StoreContext)
  const navigationItems = constructNavigationWithInPageSections(menuLinks, sectionsOnPage)
          ...
          {map(
            ({ name, isFoundInPage, node = null, link = '' }) => {
              const linkComponent = isFoundInPage
                ...
                : (<Link className={styles.navItem} to={sections.length > 1 ? `${link}/#${name}` : link}/>)
                  
              return cloneElement(
                linkComponent,
                null,
                <Heading className={styles.navItemText} textSized isLink>{name}</Heading>
              )
            },
            sections
          )}
